...
class objArea():
    ...
    def __init__(self, grid, logits, offsets, pc):
        self.grid = grid
        self.logits = logits
    ...
    def __pixel_value__(self):
        x_diff = self.offsets[0][1][1] - self.offsets[0][1][0]
        y_diff = self.offsets[0][2][1] - self.offsets[0][2][0]
    ...
    def __pixel_to_lidar__(self, pixel_area, sample_rate_x, sample_rate_y):
        ...
        min_x = self.__pixel_conv__(self.offsets[0][1][0], min_x_pixel, sample_rate_x)
        max_x = self.__pixel_conv__(self.offsets[0][1][0], max_x_pixel+1, sample_rate_x)
        
        min_y = self.__pixel_conv__(self.offsets[0][2][0], min_y_pixel, sample_rate_y)
        max_y = self.__pixel_conv__(self.offsets[0][2][0], max_y_pixel+1, sample_rate_y)
    ...
    def __build_areas__(self, pv):
        ...
        area_m_max_size = max(max_sizes[self.offsets[0][0]])
